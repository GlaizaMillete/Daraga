...
public class AudioManager : MonoBehaviour
{
    ...
    {
        // Check if the current scene is in the list of scenes to destroy the AudioManager
        if (scenesToDestroyOn.Contains(scene.name))
        {
            Debug.Log($"Destroying AudioManager in scene: {scene.name}");
            Destroy(gameObject); // Destroy the AudioManager in specified scenes
        }
        else
        {
            InitializeAudioSources(); // Refresh SFX sources for the new scene
        }
    }
