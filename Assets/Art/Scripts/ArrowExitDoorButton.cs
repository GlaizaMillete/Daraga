...
public class ArrowExitDoor : MonoBehaviour
{
    ...
    public GameObject door;  // Reference to the door GameObject for alignment
    public float offsetY = 50f;  // Vertical offset for the button position above the door
    public Vector2 exitPositionInVillage;  // Position in the VillageScene near the front door
    ...
    void OnArrowExitButtonClicked()
    {
        // Save the exit position for the VillageScene
        PlayerPrefs.SetFloat("RespawnX", exitPositionInVillage.x);
        PlayerPrefs.SetFloat("RespawnY", exitPositionInVillage.y);
        PlayerPrefs.Save();
